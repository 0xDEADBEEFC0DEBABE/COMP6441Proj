...
ULONG64 AllocMdlMemory(HANDLE iqvw64e_device_handle, ULONG64 size, ULONG64* mdlPtr) {
	...
	auto mdl = intel_driver::MmAllocatePagesForMdl(iqvw64e_device_handle, LowAddress, HighAddress, SkipBytes, pages * (ULONG64)PAGE_SIZE);
	if (!mdl) {
	...
	UINT32 byteCount = 0;
	if (!intel_driver::ReadMemory(iqvw64e_device_handle, mdl + 0x028 /*_MDL : byteCount*/, &byteCount, sizeof(UINT32))) {
...
ULONG64 kdmapper::MapDriver(HANDLE iqvw64e_device_handle, BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, mapCallback callback, NTSTATUS* exitCode) {
	...
	else if (mode == AllocationMode::AllocateMdl) {
