...
void help() {
	...
	Log(L"[+] Usage: kdmapper.exe [--free | --indPages | --mdl][--PassAllocationPtr][--copy-header]");
...
int wmain(const int argc, wchar_t** argv) {
	...
	if (mdlMode) {
		Log(L"[+] MDL allocation mode enabled" << std::endl);
	}

	if ((indPagesMode && mdlMode) || (free && indPagesMode) || (free && mdlMode)) {
		Log(L"[-] Can't use multiple allocation modes at the same time" << std::endl);
		help();
		return -1;
	}
